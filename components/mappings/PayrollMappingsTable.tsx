...
interface PayrollMappingsTableProps {
  networkId: string
}

export function PayrollMappingsTable({ networkId }: PayrollMappingsTableProps) {
  const { data, createMapping, updateMapping, deleteMapping } = useMappings(networkId, "payroll")
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Payroll Field Mappings</h3>
            ...
            {data.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-muted-foreground">
                  No payroll mappings defined. Click "Add Field" to create one.
                </TableCell>
              </TableRow>
            ) : (
              data.map((mapping) => (
