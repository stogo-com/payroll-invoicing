...
export async function GET(
  ...
  try {
    ...
    const rules = await prisma.payrollTransformationRule.findMany({
      where: {
        clientId,
        active: true,
      },
    ...
    console.error("[v0] Error fetching transformation rules:", error)
...
export async function POST(
  ...
  try {
    ...
    await prisma.payrollTransformationRule.deleteMany({
      where: { clientId },
    })
    ...
    const createdRules = await Promise.all(
      rules.map((rule, index) =>
        prisma.payrollTransformationRule.create({
          data: {
            clientId,
            ruleId: rule.id,
            name: rule.name,
    ...
    console.error("[v0] Error saving transformation rules:", error)
    return NextResponse.json(
      {
